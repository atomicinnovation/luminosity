...
Scaffold the hexagonal Rust workspace (`core` / `adapters` / `cli`) and prove it
end-to-end with a `luminosity version` subcommand — printing version plus build
metadata — built test-first, so the architecture is real and subsequent stories
have a skeleton to build on.

## Context

This is the first real Rust code in the repo and the proof of the hexagonal
architecture. It follows the modular-CLI / hexagonal architecture spike, which
decides the crate split, dispatch model, and whether the workspace starts
multi-crate or single-crate. The `version` subcommand is deliberately trivial —
its value is exercising the full path from CLI driving adapter through the core,
test-first, not the feature itself.

## Requirements

- Create the workspace per the architecture spike's recommendation: `core`
  (domain + ports as traits + application), `adapters` (port implementations),
  `cli` (composition root / driving adapter), with the inward dependency
  direction enforced (the Cargo graph plus the cargo-deny ban-list keeping infra
  crates out of `core`).
- Implement a `luminosity version` subcommand, built test-first, that prints the
  ...
  approach — the mechanism is the implementer's choice).
...
- Wire the crates into the component-based `mise` task tree (`core:`, `adapters:`,
  `cli:`) established by the toolchain story.
...
- [ ] The workspace has `core`, `adapters`, and `cli` crates (or the spike's
      recommended starting shape), with `core` depending on no adapter or I/O
      crates — a violation fails to compile and/or trips cargo-deny.
...
- Whether the workspace starts multi-crate or single-crate (hexarch-style) is
  decided by the architecture spike, not here.
- The build-metadata injection mechanism (build script vs a crate like vergen) is
  an implementation choice, not fixed here.

## Dependencies

- Blocked by: the modular-CLI / hexagonal architecture spike (0002, decides the
  crate split and dispatch); the toolchain story (0006, provides the per-crate
  checks) is paired with this story.
