...
- Tooling versions (uv, python, gh, rust, cargo-nextest, cargo-llvm-cov, shellcheck, shfmt, actionlint) are pinned in `mise.toml`; `mise` provisions them. Minimum supported Claude Code for the plugin itself is **v2.1.144** (subagent skill-preload mechanism).
